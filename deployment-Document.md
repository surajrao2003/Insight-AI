...
```bash
docker login insightai.azurecr.io
```
...
```bash
docker tag insightai:latest insightai.azurecr.io/insightai:v1
```
...
```text
insightai.azurecr.io/insightai    v1
```
...
```bash
docker push insightai.azurecr.io/insightai:v1
```

Verify image push.

```bash
az acr repository list --name insightai --output table
```
