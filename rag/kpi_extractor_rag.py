...
class FinancialMetrics(BaseModel):
    ...
    @field_validator(*_MONETARY_FIELDS, mode="after")
    @classmethod
    def format_as_dollars_in_millions(cls, value: str | int | None) -> str | int | None:
        """Normalize bare numbers (e.g. from models that ignore formatting instructions) to "$X,XXX million"."""
        if isinstance(value, str) and value.strip().lstrip("-").isdigit():
            value = int(value)

        if isinstance(value, int):
            sign = "-" if value < 0 else ""
            return f"{sign}${abs(value):,} million"
...
- Each financial value must be a single number or string for fiscal year {year} only, not a breakdown across years.
- Format every financial value as a dollar string in millions, e.g. "$391,035 million"
  (convert the reported figure to millions first if the report uses a different unit).
...
def main() -> None:
    # Run this file directly only to test kpi_extractor_rag.py
    company = "Apple"
    year = 2024
